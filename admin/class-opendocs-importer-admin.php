...
class OpenDocs_Importer_Admin {
	...
	public function getCollectionMetaData() {
		...
		$collectionID       = $_POST['data'];
		$reload             = false;
		$collectionMetaData = get_transient( 'odocs_metadata_' . $collectionID );
		if ($collectionMetaData === false || !is_array($collectionMetaData)) :
            $reload = true;
		endif;
        if ($reload) :
	...
	public function cronImportPostsCallback( $data, $itemID ) {
		// error_log('OpenDocs: cronImportPostsCallback: $itemID: '. print_r($itemID,true));
	    $wp_class = new Wordpress_IDocs();
	...
	public function loadPostSelector() {
		...
		$reload = false;
		$collectionMetaData = get_transient( 'odocs_metadata_' . $collectionID );
		if ($collectionMetaData === false):
			$reload = true;
		else:
		    if (!is_array($collectionMetaData)) :
                $reload = true;
			elseif (count($collectionMetaData) == 0):
                $reload = true;
			endif;
        endif;
...
rivate function getWeekDays() {
	 ...
	 * 1. Outstanding URL attachments for all posts
	 * Get any outstanding items which still need a url adding.
	 *
	 * 2. Import any new items for cron jobs
	 * - Get all items Do a lookuplookup
	...
	public function cronImportCallback() {
		...
		foreach ( $cron_schedules as $cron_schedule ) :
            ...
            ////// !!!!!!!!
			error_log( "OpenDocs: cronImportCallback: Check: " . print_r( $collectionID, true ) );
			$cronCondition = 0;

            // Check if we are creating new items.
            if ( $cronCondition === 1 ) :
			    ...
			    error_log( "OpenDocs: cronImportCallback: ".count($newItems)." new Items: " . print_r( $newItems, true ) );
				if ( count( $newItems ) >= 1 ) :
	            ...
	            error_log( "OpenDocs: cronImportCallback: Checking for empty items in job id " . print_r( $cron_schedule['ID'], true ) );
	            // If we're not doing the initial import, check all the existing items to see if a URL field needs updating
                if ($existingItemCount > 0) :
                    $meta_key = '';
	                foreach ($cron_schedule['mapping'] as $mapping) :
                        // Get the expected URL field
		                if ($mapping['field_name'] == 'full_text_url') {
			                $meta_key = $mapping['field_id'];
		                }
	                endforeach;
                    if ($meta_key !== '') {
	                    $itemIDs = implode( ',', $existingItems );
                        $emptyItems = $wp_class->getPostsWithoutFile( $itemIDs, $meta_key );
                        error_log( 'OpenDocs: cronImportCallback: Found  ' . count( $emptyItems ) . ' items with no URL in field ' . $meta_key );
	                    if (count( $emptyItems ) > 0) {
		                    $itemObj = new XML_IDocs_Query();
		                    $itemObj->setTimeout( 600 );
		                    $itemFiles = $itemObj->getItemFiles( $emptyItems, $cron_schedule['mapping'], array_keys( $emptyItems ) );
	                    }
                    }
	...
	public function admin_menu_callback() {
		require_once( WP_PLUGIN_DIR .  '/opendocs/admin/partials/opendocs-importer-admin-display.php' );
	}

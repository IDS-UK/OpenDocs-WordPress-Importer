(function ($) {
    ...
    $(function () {
            ...
            if ($(".publish-status").data('loaded') !== 'yes') {
                ...
                var promise = $.ajax({
                    ...
                    success: function (response) {
                        ...
                        // console.log('OpenDocs: Updated job with cron_id: ' + response);
                        $('#tab-0').data('cron-id', response);
                    }
                });

                promise.then(function () {
                    //console.log("OpenDocs: Now get the list of items for: " + $("#tab-0").data("cron-id"));
                });
                    ...
                    success: function (response) {
                        ...
                        for (var i = 0; i <= itemsArray.length; i += perPage) {
                            ...
                            for (var j = 0; j < tempArray.length; j++) {
                                ...
                                // console.log('OpenDocs: Row: ', tempArray[j]);
        ...
        $(".btn_prev .opendoc_btn, .btn_cancel .opendoc_btn").on('click', function (e) {
            ...
            //console.log("OpenDocs: Current Page: " + currentPage);
        ...
        $("#opendocs_import").on('click', function (e) {
            ...
            if (toImportCount > 0) {
                ...
                $.ajax({
                    ...
                    timeout: 0,
                    beforeSend: function () {
                        //console.log('OpenDocs: Prepare to store to imported items');
                        $(".form-wrap").hide();
            ...
            // console.log('OpenDocs: allItemsImported: ', allItemsImported);
            if (allItemsImported) {
        ...
        function addIgnoredItems(newIgnoredItems) {
            ...
            */
            //console.log('OpenDocs: addIgnoredItemIds', JSON.stringify(newIgnoredItems));
            var ajaxdata = {
            ...
            $.ajax({
                ...
                timeout: 0,
                beforeSend: function () {
                    //console.log('OpenDocs: About to update newIgnoredItems');
                },
                success: function (response) {
                    var IgnoredItemIds = [];
                    $.each(response, function (index, element) {
        ...
        function checkForImportedPosts(totalRecords, newItemCount, postType, jobID) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    console.log("OpenDocs: Item Count: " + newItemCount + ", Imported: " + response + " into type " + postType + " for job " + jobID);
        ...
        function checkIfPostOnlyImportDone() {
            ...
            $.ajax({
                ...
                processed: 0,
                success: function (response) {
                    //console.log('OpenDocs: checkifdone: Got ' + response + ' of '+ this.countItemIds);
        ...
        $(".check-all").on('change', function () {
            ...
            // Get all boxes on current page
            var checkboxes = $(".items[id*='" + currentPage + "']").find('input[type="checkbox"]');
            // For each one which is not readonly
            checkboxes.each(function() {
                if ($(this).prop('readonly') == false) {
                    // Check if the ID contains the clicked type (ignore or import)
                    var id = $(this).attr('id');
                    // console.log('ID: '+id+' and ' + id.indexOf(checkboxType));
                    if (id.indexOf(checkboxType) !== -1) {
                        $(this).prop("checked", true);
                    } else {
        ...
        $(".btn_save .opendoc_btn").on('click', function (e) {
            ...
            console.log('OpenDocs: Jobname = ' + jobName);
            var cronid = $('#tab-0').data('cron-id');
            var dataToImport = buildSaveData(cronid);
            var ajaxdata = {
                'action': 'updateImportJob',
        ...
        function buildMappingTable(jobLinkOBJ) {
            ...
            $.each(savedMappings, function (index, elem) {
                var delMapping = '';
                //console.log("OpenDocs: buildMappingTable: " + elem.field_id + ", Mapping Field: " + elem.field_name);
        ...
        function buildOptionsTable(jobLinkOBJ) {
            ...
            $(importRowOBJs).each(function (index, el) {
                ...
                //console.log('OpenDocs: Current Cron id for list: ' + importRowOBJ.find('.imported-items').data('cronid'));
    ...
    $(window).load(function () {
        console.log('opendocs-import-admin.js v3');
